	...
	$.fn.eCalendar = function( options ) {
		...
		this.init = function() {
			...
			if( options.eventsContainer == '#hb-event-list' && el.next('#hb-event-list').length == 0 ) {
				...
				// comentado para no crear el div de eventos
		...
		this.initDays = function() {
			...
			for( var i = 1; i < dayCount + firstDay; i++ ) {
				...
				if( i < firstDay )
					...
					if (options.currentMonth == currentMonth) {
						if ( dayNumber == currentDay ) {
							console.log(dayNumber, "adentro");
							dayClassCurrent = " current_day";
						} else {
							dayClassCurrent = "";
							console.log(dayNumber, "afuera");
						}

					}


					calendar += '<span href="javascript:;" class="' + dayClass + dayClassCurrent + '" ' + dayData + '>' + dayNumber + '</span>';
