...
(function ($) {
  ...
  function createTemplate() {
    ...
    return isLabelMode ? templateSearch + '<div class="dropdown-main">{{ul}}</div><div class="dropdown-display-label"><div class="dropdown-chose-list"></div></div>' 
    : 
    '<a href="javascript:;" class="dropdown-display" tabindex="0"><span class="dropdown-chose-list"></span><a href="javascript:;"  class="dropdown-clear-all" tabindex="0">\xD7</a></a><div class="dropdown-main">' + templateSearch + '{{ul}}</div>';
  }
  ...
  function selectToDiv(str) {
    ...
    result = result.replace(/<option(.*?)<\/option>/gi, function (matcher) {
      ...
      return '<div class="cont_li" style="position: relative;"><span class="ver_det" onclick="show_mod(' + value + ')"><i class="fa fa-search"></i></span><li ' + (isDisabled ? ' disabled' : ' tabindex="0"') + ' data-value="' + (value || '') + '" class="dropdown-option ' + (isSelected ? 'dropdown-chose' : '') + '" ' + extendAttr + '>' + (name ? name[1] : '') + '</li></div>';
    });
  ...
  function objectToSelect(data) {
    ...
    $.each(data, function (index, val) {
      ...
      if (isSelected) {
        name.push('<span class="dropdown-selected fle-col"><span>' + val.name + '</span><span class="ver_det" onclick="show_mod(' + val.id + ')"><i class="fa fa-search"></i></span><i class="del" data-id="' + val.id + '"></i></span>');
        selectAmount++;
      }
      // 判断是否有分组
      if (hasGroup) {
    ...
    multiChoose: function (event, status) {
      ...
      $.each(_config.data, function (key, item) {
        ...
        if (item.selected) {
          ...
          _dropdown.name.push('<span class="dropdown-selected">' + item.name + '<span class="ver_det" onclick="show_mod(' + item.id + ')"><i class="fa fa-search"></i></span><i class="del" data-id="' + item.id + '"></i></span>');
        }
      });
