...
$consulta = 
  "
  SELECT 
    IFNULL(SUM(viv.valor_viv),0) AS total
  FROM
    cotizacion_cotizacion AS cot
    INNER JOIN vendedor_jefe_vendedor AS vjefe ON vjefe.id_vend = cot.id_vend
    INNER JOIN vivienda_vivienda AS viv ON viv.id_viv = cot.id_viv
  WHERE
    cot.id_est_cot IN (1,4,5,6,7) AND
    vjefe.id_usu = ".$_SESSION["sesion_id_panel"]."
  ";
$conexion->consulta($consulta);
$fila = $conexion->extraer_registro_unico();
...
$consulta_sin_seg = 
    ...
    ";
$conexion->consulta($consulta_sin_seg);
$cantidad_sin_seg = $conexion->total();

//------- Cotizaciones con seguimiento atrasado --------
...
$consulta = 
    ...
    WHERE
    	...
    	jef.id_vend = cot.id_vend AND
    	jef.id_usu = ".$_SESSION["sesion_id_panel"]."
    ";

$conexion->consulta($consulta);
$hayseg = $conexion->total();
if ($hayseg>0) {
	...
	if(is_array($fila_consulta_seg)){
	    foreach ($fila_consulta_seg as $fila_seg) {
	        ...
	        if($fecha_proximo < $hoy){
	            $acumulado_atrasado_seg = $acumulado_atrasado_seg + 1;
	        }
	    }
	}
}
    ...
    <div class="box box-widget widget-user-2">            
      ...
      <div class="box-footer no-padding">
        <ul class="nav nav-stacked">
          ...
          <li><a href="<?php echo _MODULO?>informe/venta_cotizacion_venta.php" target="_blank">Cotizaciones sin Seguimiento<span class="pull-right badge bg-red"><?php echo number_format($cantidad_sin_seg, 0, ',', '.');?></span></a></li>
