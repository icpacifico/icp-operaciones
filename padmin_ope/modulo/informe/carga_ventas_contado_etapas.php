...
if(is_array($fila_consulta)){
	foreach ($fila_consulta as $fila) {
		...
		$consulta_actuales = "SELECT id_eta FROM etapa_etapa WHERE id_for_pag = 2 and numero_real_eta < ".$filamax['numero_real_eta']." ORDER BY numero_real_eta ASC";
		$conexion->consulta($consulta_actuales);

		$fila_consulta_ac = $conexion->extraer_registro();
		if(is_array($fila_consulta_ac)){
		    foreach ($fila_consulta_ac as $fila_eactuales) {
		    	$id_eta = $fila_eactuales['id_eta'];

		    	echo $id_eta."<br>";

		    	$consulta_tiene = "SELECT id_eta_ven FROM venta_etapa_venta WHERE id_eta = ".$id_eta." AND id_ven = ".$id_ven."";
				$conexion->consulta($consulta_tiene);
				$tiene_la_etapa = $conexion->total();
				if($tiene_la_etapa>0){
					echo "la tiene, no hace nada<br>";
				} else {
					echo "-------------->no tiene la etapa, se debe insertar<br>";
					$fecha = "2021-01-01 10:10:10";
					$consulta = "INSERT INTO venta_etapa_venta VALUES(?,?,?,?,?,?,?)";
					$conexion->consulta_form($consulta,array(0,$id_ven,$id_eta,1,$fecha,$fecha,""));
				}
