    ...
    foreach ($fila_consulta_cierre as $fila_cierre) {
		...
		$html .= '<table class="sin-borde">
		    <tr>
				...
				<td style="text-align: center;"><h2>Detalle Bono Operaciones: '. utf8_encode($nombre_usu." ".$apellido1_usu." ".$apellido2_usu).'</h2><h6>Período: '. $id_mes.'/ '.utf8_encode($anio_cie).' </h6></td>
				<td style="width: 20%;">
					<table class="hoy">
						<tr>
		...
		$html .= '<table class="liquida">
			...
			<tbody>
		        ...
		        if(is_array($fila_consulta_detalle)){
		            foreach ($fila_consulta_detalle as $fila_det) {
		                ...
		                $html .= '<tr class="lista">
		                	...
		                	<td>'.utf8_encode($fila_det['nombre_viv']).'</td>
		                    ...
		                    <td colspan="3">'.number_format($fila_det['escritura_monto_comision_operacion_ven'], 3, ',', '.').'</td>
		                    <td colspan="2">'.number_format($monto_comision_escritura, 0, ',', '.').'</td>';
				...
				$html .= '<tr class="separa total">
					...
					<td colspan="3" style="text-align: center">'.number_format($monto_uf_acumulado_escritura, 2, ',', '.').'</td>
					<td colspan="2" style="text-align: center">'.number_format($monto_acumulado_escritura, 0, ',', '.').'</td>
					<td></td>
