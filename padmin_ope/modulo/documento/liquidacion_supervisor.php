...
<body>
	<a class="btn no-print" href="liquidacion_supervisor_pdf.php?id=<?php echo $id_cierre; ?>&id_usu=<?php echo $id_usuario; ?>" target="_blank">PDF</a>
<?php 
$consulta = 
    "
    SELECT
    ...
    foreach ($fila_consulta_cierre as $fila_cierre) {
        ...
        <table class="sin-borde">
            <tr>
        		...
        		<td style="text-align: center;"><h2>Detalle Comisión Supervisor de Ventas: <?php echo utf8_encode($nombre_usu." ".$apellido1_usu." ".$apellido2_usu);?></h2><h6>Período: <?php echo $id_mes;?>/<?php echo utf8_encode($anio_cie);?></h6></td>
        		<td style="width: 20%;">
        			<table class="hoy">
        				<tr>
        ...
        <table class="liquida">
        	...
        	<tbody>
        		<tr class="cabecera">
        			...
        			<td style="width: 7%" class="bl-1">Total<br><?php echo $comision;?></td>
                ...
                if($cantidad_bono > 0){
                    ...
                    if(is_array($fila_consulta_bono)){
                        foreach ($fila_consulta_bono as $fila_bono) {
                            ...
                            <tr class="separa">
                                <td colspan="2"></td>
                                <td colspan="2"><?php echo utf8_encode($fila_bono['nombre_bon_cie']);?></td>
                ...
                if(is_array($fila_consulta_detalle)){
                    foreach ($fila_consulta_detalle as $fila_det) {
                        ...
                        <tr class="lista">
                            ...
                            <td><?php echo number_format($fila_det['promesa_bono_precio_supervisor_ven'], 2, ',', '.');?></td>
                            <td><?php echo number_format($fila_det['escritura_bono_precio_supervisor_ven'], 2, ',', '.');?></td>
