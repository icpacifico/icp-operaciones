    ...
    foreach ($fila_consulta_cierre as $fila_cierre) {
        ...
        $html .= '<table class="sin-borde">
            <tr>';
        		...
        		<td style="text-align: center;"><h2>Detalle Comisión Supervisor de Ventas: '.utf8_encode($nombre_usu." ".$apellido1_usu." ".$apellido2_usu).'</h2><h6>Período: '.$id_mes.'/'.utf8_encode($anio_cie).'</h6></td>
        		<td style="width: 20%;">
        			<table class="hoy">
        				<tr>
        	...
        	<tbody>
                ...
                $consulta = 
                    ...
                    FROM
                        ...
                        INNER JOIN venta_venta AS ven ON ven.id_supervisor_ven = usu.id_usu
                        INNER JOIN vivienda_vivienda AS viv ON viv.id_viv = ven.id_viv
                    ...
                    foreach ($fila_consulta_detalle as $fila_det) {
                        ...
                        $html .= '<tr class="lista">
                            ...
                            if($cantidad_estado_promesa > 0){
                            	$monto_uf_acumulado_promesa = $monto_uf_acumulado_promesa - $fila_det['promesa_monto_comision_supervisor_ven'];
                                ...
                                $html .= '<td>'.number_format($fila_det['promesa_monto_comision_supervisor_ven'], 2, ',', '.').'</td>';
                            ...
                            if($cantidad_estado_escritura > 0){
                            	$monto_uf_acumulado_escritura = $monto_uf_acumulado_escritura - $fila_det['escritura_monto_comision_supervisor_ven'];
                                ...
                                $html .= '<td>'.number_format($fila_det['escritura_monto_comision_supervisor_ven'], 2, ',', '.').'</td>';
                           	}
                            else{
                                $html .= '<td></td>';
                            }
                ...
                if(is_array($fila_consulta_detalle)){
                    foreach ($fila_consulta_detalle as $fila_det) {
                        ...
                        $html .= '<tr class="lista">
                            ...
                            <td class="nombre">'.utf8_encode($fila_det['nombre_pro']." ".$fila_det['apellido_paterno_pro']." ".$fila_det['apellido_materno_pro']).'</td>
                            <td>'.date("d/m/Y",strtotime($fila_det['fecha_ven'])).'</td>
                            <td>'.number_format($fila_det['total_comision_supervisor_ven'], 2, ',', '.').'</td>
                            <td>'.number_format($fila_det['promesa_monto_comision_supervisor_ven'], 2, ',', '.').'</td>
                            <td>'.number_format($fila_det['escritura_monto_comision_supervisor_ven'], 2, ',', '.').'</td>';
                ...
                if(is_array($fila_consulta_detalle)){
                    foreach ($fila_consulta_detalle as $fila_det) {
                        ...
                        $html .= '<tr class="lista">
                        	<td>'.utf8_encode($alias_con).'</td>
                        	<td>'.number_format($fila_det['monto_ven'], 2, ',', '.').'</td>
                            <td class="nombre">'.utf8_encode($fila_det['nombre_pro']." ".$fila_det['apellido_paterno_pro']." ".$fila_det['apellido_materno_pro']).'</td>
                            <td></td>
                            <td>'.number_format($fila_det['total_bono_precio_supervisor_ven'], 2, ',', '.').'</td>
                            <td>'.number_format($fila_det['promesa_bono_precio_supervisor_ven'], 2, ',', '.').'</td>
                            <td>'.number_format($fila_det['escritura_bono_precio_supervisor_ven'], 2, ',', '.').'</td>';
                            ...
                            if($cantidad_estado_promesa > 0){
                            	$monto_uf_acumulado_promesa = $monto_uf_acumulado_promesa + $fila_det['promesa_bono_precio_supervisor_ven'];
                        	...
                        	if($cantidad_estado_escritura > 0){
                        		$monto_uf_acumulado_escritura = $monto_uf_acumulado_escritura + $fila_det['escritura_bono_precio_supervisor_ven'];
...
$nombre = 'documentos/liquidacion-supervisor-'.date('dmYHi').'.pdf';
