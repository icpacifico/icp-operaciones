...
if (empty($descuento)) {
	...
	$descuento_adicional = $cotizacion->cotizacion_descuento_adicional($descuento);
...
if($precio_descuento == 1){
    $descuento_precio = ($valor_viv * $porcentaje_descuento) / 100;
}
...
if($cantidad_estacionamiento > 0){
	$monto_estacionamiento = $cotizacion->cotizacion_estacionamiento($_POST["estacionamiento"]);
}
if($cantidad_bodega > 0){
	$monto_bodega = $cotizacion->cotizacion_bodega($_POST["bodega"]);
}
