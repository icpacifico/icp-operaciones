...
$pie = $_POST["pie"];
$premio = $_POST["premio"];
$total_vivienda = $_POST["total_vivienda"];
...
if (empty($monto_vivienda) || $monto_vivienda > $valor_viv || empty($fecha) || empty($precio_descuento) || empty($forma_pago) || empty($pie) || empty($aplica_pie)) {
    ...
    <div class="col-sm-12" style="margin-top: 20px;">
        ...
        /*if (empty($total_vivienda) || $total_vivienda == '')  {
            if($total_vivienda == ''){
                ?>
                <h6>- N° Bien Inscrito</h6>
                <?php 
            }
...
if($cantidad_estacionamiento > 0){
    $consulta = 
        ...
        WHERE 
            id_esta IN (".implode(',',$_POST["estacionamiento"]).") 
        ";
...
if($cantidad_bodega > 0){
    $consulta = 
        ...
        FROM 
            bodega_bodega
        WHERE 
            id_bod IN (".implode(',',$_POST["bodega"]).") 
        ";
    $conexion->consulta($consulta);
    $fila = $conexion->extraer_registro_unico();
...
// echo "Precio Depto ".number_format($monto_vivienda, 2, ',', '.')."<br>";
// echo "Fecha Promesa ".date("d/m/Y",strtotime($fecha))." UF ".number_format($valor_uf, 2, ',', '.')."<br>";
...
if(!empty($descuento)){ //desc adicional
    $consulta = 
        "
        SELECT
            monto_des
        ...
        WHERE
            id_des = ?
        ";
    $conexion->consulta_form($consulta,array($descuento));
    $fila = $conexion->extraer_registro_unico();
    $monto_descuento = $fila['monto_des'];
...
$fila = $conexion->extraer_registro_unico();
// echo "Forma de Pago ".utf8_encode($fila['nombre_for_pag'])."<br>";
...
// echo "% Pie ".utf8_encode($fila['valor_pie_ven'])."<br>";
...
<div class="col-md-4">
	...
	<?php
	} else if($aplica_pie==2 && ($precio_descuento == 1 || !empty($descuento))) {
	 ?>
	<div class="info">--</div>
	<?php } else {
	?>
	...
	<div class="info"><b>% Pie:</b> <?php echo utf8_encode($fila['valor_pie_ven']); ?></div>
<?php
...
if($cantidad_estacionamiento > 0){
    ...
    if(is_array($fila_consulta)){
        foreach ($fila_consulta as $fila) {
            ...
            <?php
            // echo utf8_encode($fila['nombre_esta'])." - ".number_format($valor_esta, 2, ',', '.')."UF <br>";
        }
    }
}
...
if($cantidad_bodega > 0){
    ...
    if(is_array($fila_consulta)){
        foreach ($fila_consulta as $fila) {
            ...
            <?php
            // echo utf8_encode($fila['nombre_bod'])." - ".number_format($valor_bod, 2, ',', '.')."UF <br>";
        }
    }
}
...
//-------- CALCULOS DE MONTOS
if($precio_descuento == 1 || !empty($descuento)){  //si aplica precio con descuento
    // echo "DESCUENTOS <br>";
    if($precio_descuento == 1 && $aplica_pie == 1){
        ...
        <?php
        // echo "Descuento ".number_format($total_precio_descuento, 2, ',', '.')." UF <br>";
        // echo "Descuento Peso $".number_format($total_precio_descuento_peso, 2, ',', '.')."<br>";
    }   
    if(!empty($descuento)){
    	?>
		<div class="info"><b>Descuento Adicional:</b> <?php echo number_format($monto_descuento, 2, ',', '.');?> UF</div>
        <?php
        // echo "Descuento Adicional ".number_format($monto_descuento, 2, ',', '.')." UF <br>";
    }
    if($aplica_pie == 2){
...
if($cantidad_estacionamiento > 0){
    ...
    <?php
    // echo "Monto Total Estacionamientos Adicionales ".number_format($monto_estacionamiento, 2, ',', '.')." UF <br>";
...
if($cantidad_bodega > 0){
    ...
    <?php
    // echo "Monto Total Bodegas Adicionales ".number_format($monto_bodega, 2, ',', '.')." UF <br>";
}
...
<?php
// echo "Monto Reserva ".number_format($monto_reserva, 2, ',', '.')." UF <br>";

//----- PIE

// echo "Monto Pie ".number_format($monto_pie_sin_reserva, 2, ',', '.')." UF <br>";
if($aplica_pie == 1){ //descuenta al pie
    // echo "<br>".$monto_pie_sin_reserva."<br>";
    // $monto_pie_con_descuento = $monto_pie_sin_reserva - $total_precio_descuento - $monto_descuento;
    $monto_pie = $monto_vivienda_total * ($porc_pie / 100);
    $monto_pie_sin_reserva = $monto_pie - $monto_reserva;
	if($precio_descuento == 1 || !empty($descuento)){ 
...
<div id="contenedor_boton" class="box-footer">
    <button type="submit" class="btn btn-primary pull-right" id="guardar">Pasar a Promesa</button>
</div>