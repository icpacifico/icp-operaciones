...
$cantidad_mes_actual = $propietario->calcula_cantidad_mensual_mails($ids_cant);

if($cantidad_mes_actual == 1) {
	$titulo_plantilla = $propietario->recupera_titulo_campana($tipo_mail);

	$describe_masiva = utf8_decode("Envió masivo mail ".utf8_encode($titulo_plantilla));

	$valida_repetido = $propietario->verifica_envio_repetido($_SESSION["sesion_id_panel"],$fecha,$asunto,$enlace_imagen,$titulo_plantilla,$ids_cant,$ids);
	...
	if($valida_repetido==0) {
		...
		$cantidad = $ids_cant - 1;
		$contador = 0;
		$propietario->propietario_insert_campana($_SESSION["sesion_id_panel"],$fecha,$asunto,$enlace_imagen,$titulo_plantilla,$ids_cant);
		...
		$ejecucion_masiva = $propietario->propietario_insert_envio_masivo($ultimo_id,$id_emp,$cantidad,$asunto,$enlace_imagen,$tipo_mail,$nombre_vendedor,$email_vendedor,$_SESSION["sesion_id_panel"],$fecha,$describe_masiva);
