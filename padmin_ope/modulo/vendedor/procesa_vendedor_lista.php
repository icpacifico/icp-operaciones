...
$consulta_nueva = "
	SELECT lis.nombre_lis, lis.id_lis 
	FROM lista_vendedor_lista AS lis_ven, lista_lista AS lis 
	WHERE lis_ven.id_vend = ".$id."
	AND lis_ven.id_lis = lis.id_lis 
	ORDER BY lis.id_lis ASC";
...
if(is_array($fila_consulta)){
    foreach ($fila_consulta as $fila) {
    	$clientes_actuales .= $fila['id_lis'].",";
        $nombre_lis = utf8_encode($fila['nombre_lis']);
        $id_lis = $fila['id_lis'];
        $data .= '{"name": "'.$nombre_lis.'", "id":'.$id_lis.', "disabled": false, "selected": true },';
    }
}
...
if(is_array($fila_consulta)){
    foreach ($fila_consulta as $fila) {
        $nombre_lis = utf8_encode($fila['nombre_lis']);
        $id_lis = $fila['id_lis'];
        $data .= '{"name": "'.$nombre_lis.'", "id":'.$id_lis.', "disabled": false, "selected": false },';
    }
}
...
<div class="box-header">
    <h3 class="box-title"><i class="fa fa-list" aria-hidden="true"></i> Listas</h3>
</div>
