...
$etapa->etapa_insert();
$id_etapa = $etapa->recupera_id();


$campo_numero = utf8_decode($_POST["campo_numero"]);
foreach ($campo_numero as $key => $valor) {
	$etapa->etapa_campo_insert($id_etapa,2,$valor);
}

$campo_fecha = utf8_decode($_POST["campo_fecha"]);
foreach ($campo_fecha as $key => $valor) {
	$etapa->etapa_campo_insert($id_etapa,3,$valor);
}

$campo_texto = utf8_decode($_POST["campo_texto"]);
foreach ($campo_texto as $key => $valor) {
	$etapa->etapa_campo_insert($id_etapa,1,$valor);
}
